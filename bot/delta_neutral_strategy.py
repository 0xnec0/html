...
class DeltaNeutralStrategy:
    ...
    async def _wait_for_tight_spread(self, max_spread_pct: float, check_interval: float, timeout: float) -> Optional[tuple]:
        ...
        while True:
            ...
            # Calculate inter-exchange spread
            ...
            print(f"\r   [{check_count}] スプレッド: {spread_pct:.4f}% | Paradex: ${paradex_price:.4f} | Lighter: ${lighter_price:.4f} | 経過: {elapsed:.0f}秒", end='', flush=True)
            ...
            if timeout > 0:
                if elapsed >= timeout:
                    ...
                    print(f"   最終スプレッド: {spread_pct:.4f}% (目標: ≤{max_spread_pct}%)")
                    return None

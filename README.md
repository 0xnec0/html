...
**動作：**
1. スプレッドが0.02%以下になるまで待機
2. Paradexでロング（BUY）+ Lighterでショート（SELL）を同時オープン
3. ランダムな時間（2-3時間デフォルト）待機
4. スプレッドが0.02%以下になるまで待機
5. 両ポジションをクローズ（Paradex SELL + Lighter BUY）
6. 30秒待機して1に戻る
...
# スプレッド監視設定（NEW!）
SPREAD_MAX_PCT=0.02              # 各取引所の最大許容bid-askスプレッド（%）
SPREAD_CHECK_INTERVAL=2          # チェック間隔（秒）
SPREAD_CHECK_TIMEOUT=0           # タイムアウト（秒）- 0=無限待機
```

**スプレッド監視機能（NEW!）:**
- 🎯 **各取引所のbid-askスプレッド**が0.02%以下になるまで待機
- 📊 Paradex と Lighter の**両方のスプレッド**を同時監視
- 🔄 リアルタイムで各取引所のスプレッドを表示
- ⏱️ タイムアウト設定可能（0=条件達成まで無限待機）
- 💰 流動性が高い瞬間を狙って約定コストを最小化
- 📈 **ポジション開く時** と **閉じる時** の両方で監視
